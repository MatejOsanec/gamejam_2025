...
namespace BGLib.UnityExtension.Editor {
    ...
    public static class SavWav {
        ...
        public static void Save(string filepath, AudioClip clip, float start, float duration) {
            ...
            var wav = GetWav(clip, out uint length, start, duration);
            writer.Write(wav, 0, (int)length);
        }

        private static byte[] GetWav(AudioClip clip, out uint length, float start, float duration) {

            var data = ConvertAndWrite(clip, out length, out uint samples, start, duration);
        ...
        private static byte[] ConvertAndWrite(
            ...
            int sStart = (int)(start * clip.frequency * clip.channels);
            int sEnd = Math.Min(samples.Length - 1, sStart + (int)(duration * clip.frequency * clip.channels));
            ...
            length = p;
            samplesAfterTrimming = (uint)(sEnd - sStart + 1);
            return buffer;
        }
