...
namespace BGLib.UnityExtension.Editor {
    ...
    [CustomPropertyDrawer(typeof(ShaderPropertyIDFromPropertyAttribute))]
    public class ShaderPropertyIDFromPropertyAttributeDrawer :
        ShaderPropertyIDAttributeDrawer<ShaderPropertyIDFromPropertyAttribute> {
        ...
        protected virtual DataGatherResult<Renderer[]> GetTargetFromRenderer(SerializedProperty property) {
            ...
            var parentProperty = property.serializedObject.FindProperty(castedAttribute.propertyName);

            if (parentProperty.objectReferenceValue == null) {
                var message = string.Format(kPropertyIsNullMessage, property.name, castedAttribute.propertyName);
                return DataGatherResult<Renderer[]>.Fail(message);
            }

            var serialisedProperty = new SerializedObject(parentProperty.objectReferenceValue);

            return GetTargetFromRenderer(parentProperty, serialisedProperty, castedAttribute.nestedPropertyName);
        }

        protected DataGatherResult<Renderer[]> GetTargetFromRenderer(SerializedProperty parentProperty, SerializedObject serializedObject, string propertyName) {

            var foundProperty = serializedObject.FindProperty(propertyName);

            if (foundProperty == null) {
                var message = string.Format(kPropertyNotFoundMessage, parentProperty.name, propertyName);
                return DataGatherResult<Renderer[]>.Fail(message);
            }
            ...
            var errorMessage = string.Format(kPropertyIsNullMessage, parentProperty.name, propertyName);
            return DataGatherResult<Renderer[]>.Fail(errorMessage);
        }
    }
}
