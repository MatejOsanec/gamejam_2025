public class PerlinNoise {
    ...
    public static float OctavePerlin3D(float x, float y, float z, int octaves, float persistence, int repeat = 0) {
        ...
        for (int i = 0; i < octaves; i++) {
            ...
            total += Perlin3D(x * frequency, y * frequency, z * frequency, repeat) * amplitude;
            maxValue += amplitude;
    ...
    public static float Perlin3D(float x, float y, float z, int repeat) {
        ...
        float x1, x2, y1, y2;
        x1 = Lerp(Grad3D(aaa, xf, yf, zf),              // The gradient function calculates the dot product between a pseudorandom
            Grad3D(baa, xf - 1, yf, zf),                // gradient vector and the vector from the input coordinate to the 8
            u);                                     // surrounding points in its unit cube.
        x2 = Lerp(Grad3D(aba, xf, yf - 1, zf),              // This is all then lerped together as a sort of weighted average based on the faded (u,v,w)
            Grad3D(bba, xf - 1, yf - 1, zf),                // values we made earlier.
            u);
        y1 = Lerp(x1, x2, v);

        x1 = Lerp(Grad3D(aab, xf, yf, zf - 1),
            Grad3D(bab, xf - 1, yf, zf - 1),
            u);
        x2 = Lerp(Grad3D(abb, xf, yf - 1, zf - 1),
